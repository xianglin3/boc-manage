...
$(document).ready(function(){
    ...
    var selectPage = {
        ...
        drawBody:function(){
            ...
            $("div.user_info span[data-val]").each(function(i,v){
                var html = me.data['user_info'] || {"name":"当前用户未注册","hire_date":"","work_group":""};
    ...
    //课表和人员表格(类)
    function Course_Human(ele,url,thead,add_btn,sel,isAllowDelete,data_time){
        this.ele = ele;
        this.url =url;
        this.isFist = true;
        this.tHead = thead;
    ...
    Course_Human.prototype.addEvent = function(){
        ...
        $(me.add_btn).click(function(){
            ...
            $(me.ele+" tr[data-id] input.mod_input").each(function(i,v){
                var name = $(v).attr("name");
                if(me.dataTime[name]){
                    $(v).addClass("datatime_input");
                    $(v).inputmask(me.dataTime[name]);
                    $(v).blur(function(){
                        var val = $(this).val();
                        if(/[a-z]/.test(val)){
                            $(this).val("");
